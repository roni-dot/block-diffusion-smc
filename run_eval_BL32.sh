...
# run_eval "corrected_kv_cache_baseline_N1_alpha1_T0_S${SAMPLE}" "${N_PARTICLES_BASE}" "${ALPHA_BASE}" "${TEMPERATURE_BASE}"
...
# run_eval "equation8_smc_N4_alpha${ALPHA}_T05_S${SAMPLE}" 4 "${ALPHA}" "${TEMPERATURE}"

# # 5. Power-SMC N=4 (matches Power-SMC paper: temp=0.25, alpha=4.0)
# run_eval "equation8_smc_N4_alpha${ALPHA_POWERSMC}_T025_S${SAMPLE}" 4 "${ALPHA_POWERSMC}" "${TEMPERATURE_POWERSMC}"

# 6. Power-SMC N=8 T05
run_eval "equation8_smc_N${N_PARTICLES}_alpha${ALPHA}_T05_S${SAMPLE}" "${N_PARTICLES}" "${ALPHA}" "${TEMPERATURE}"

# 7. Power-SMC N=8 (matches Power-SMC paper: temp=0.25, alpha=4.0)
run_eval "equation8_smc_N${N_PARTICLES}_alpha${ALPHA_POWERSMC}_T025_S${SAMPLE}" "${N_PARTICLES}" "${ALPHA_POWERSMC}" "${TEMPERATURE_POWERSMC}"

# 6. Power-SMC N=12 T05 (added N=12 to see if more particles helps)
run_eval "equation8_smc_N12_alpha${ALPHA}_T05_S${SAMPLE}" 12 "${ALPHA}" "${TEMPERATURE}"

# 7. Power-SMC N=12 (matches Power-SMC paper: temp=0.25, alpha=4.0)
run_eval "equation8_smc_N12_alpha${ALPHA_POWERSMC}_T025_S${SAMPLE}" 12 "${ALPHA_POWERSMC}" "${TEMPERATURE_POWERSMC}"
